...
PR #35 was merged before the rendered visual and Product Owner gates above were completed. Treat those gates as post-merge verification that must be closed before the VS-05 customer discovery experience is promoted to the Netlify production source branch or treated as release-ready.

## Known slice boundary

The VS-05 Landing cards route to `/packages/[departureId]`, but the existing package-detail surface is not yet backed by the authoritative public contract for arbitrary published departures. That migration belongs to VS-06. Do not treat the current static preview detail data as authoritative VS-05 output, and do not treat VS-05 as a complete customer browse-to-detail journey until VS-06 closes this boundary.
