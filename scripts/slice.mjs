...
function completedSlices() {
  return existsSync(completedPath)
    ? new Set(readJson(completedPath).completed ?? [])
    : new Set();
}
...
function validateManifest(manifest, allIds, completed) {
  ...
  for (const key of requiredStrings) {
    if (typeof manifest[key] !== "string" || manifest[key].trim() === "") {
      errors.push(`${manifest.id ?? "unknown"}: ${key} is required`);
    }
  }
  if (!/^VS-\d{2}$/.test(manifest.id ?? "")) {
    errors.push(`${manifest.id ?? "unknown"}: id must match VS-00`);
  }
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(manifest.slug ?? "")) {
    errors.push(`${manifest.id ?? "unknown"}: slug must be kebab-case`);
  }
  ...
  for (const key of [
    ...
    if (!Array.isArray(manifest[key]) || manifest[key].length === 0) {
      errors.push(`${manifest.id}: ${key} must be a non-empty array`);
    }
  }

  for (const dependency of manifest.dependsOn ?? []) {
    if (!completed.has(dependency) && !allIds.has(dependency)) {
      errors.push(`${manifest.id}: unknown dependency ${dependency}`);
    }
  }

  for (const gate of requiredGates) {
    if (!(manifest.qualityGates ?? []).includes(gate)) {
      errors.push(`${manifest.id}: missing required quality gate ${gate}`);
    }
  }
  ...
  if (typeof manifest.rendered !== "object" || manifest.rendered === null) {
    ...
    errors.push(
      `${manifest.id}: rendered.testFile is required when rendered review is enabled`,
    );
  }
...
function validateAll() {
  ...
  for (const { value } of loaded) {
    errors.push(...validateManifest(value, allIds, completed));
  }

  for (const { value } of loaded) {
    for (const key of ["specPath", "checklistPath"]) {
      if (!existsSync(join(root, value[key]))) {
        errors.push(`${value.id}: missing ${value[key]}`);
      }
    }
  }
...
function findManifest(idOrBranch, optional = false) {
  ...
  const match = manifests().find(({ value }) => {
    ...
    return (
      ...
      token.includes(value.slug)
    );
  });
  if (!match && !optional) {
    throw new Error(`No slice manifest matches '${idOrBranch}'`);
  }
...
function scaffold(id) {
  ...
  console.log(
    `branch: agent/${manifest.id.toLowerCase().replace("-", "")}-${manifest.slug}`,
  );
  console.log(
    "next: keep Draft; apply 'certify' only after implementation and local review are complete",
  );
}
...
function status(id) {
  ...
  console.log(
    `${manifest.id} ${manifest.title}: ${complete} complete, ${open} open`,
  );
}
...
function resolveManifest(args) {
  ...
  const output = {
    ...
    artifact_name: manifest
      ? `${manifest.id.toLowerCase()}-rendered-review`
      : "rendered-review",
