...
public sealed class DocumentRetentionWorker(IServiceScopeFactory scopes,TimeProvider clock) : BackgroundService
{
    ...
    private async Task DeleteDue(CancellationToken ct)
    {
        ...
        foreach(var item in due){await storage.DeleteAsync(item.ObjectKey,ct);item.State=SubmissionState.Deleted;item.ObjectKey=$"deleted:{item.Id:N}";db.Audit.Add(new(){Id=Guid.NewGuid(),SubmissionId=item.Id,Action="object_deleted",ActorId="system",Purpose="retention schedule",OccurredAtUtc=now});} await db.SaveChangesAsync(ct);
