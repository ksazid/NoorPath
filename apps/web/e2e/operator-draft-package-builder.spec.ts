...
test("saved package draft renders a private customer-style preview", async ({ page }) => {
  ...
  await page.route(`**/api/v1/operator/departures/${departureId}`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        summary: "A guided Umrah journey with confirmed accommodation disclosures.",
        origin: "Delhi (DEL)",
        departureDate: "2027-01-10",
        returnDate: "2027-01-21",
        makkah: { hotelName: "Makkah Stay", classification: "4 star", distanceDisclosure: "850 m from Masjid al-Haram", nights: 6 },
        madinah: { hotelName: "Madinah Stay", classification: "4 star", distanceDisclosure: "450 m from Al-Masjid an-Nabawi", nights: 5 },
        travel: { routeSummary: "Delhi → Jeddah → Makkah → Madinah", details: "Intercity travel by bus" },
        inclusions: ["Visa included", "Breakfast, lunch and dinner", "Intercity travel"],
  ...
  await expect(page.getByText("Visa included", { exact: true })).toBeVisible();
  await expect(page.getByText("Private draft · not visible to customers", { exact: true })).toBeVisible();
  await expect(page.getByRole("link", { name: "Review publication" })).toBeVisible();
