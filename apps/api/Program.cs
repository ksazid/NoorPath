...
builder.Services.AddDbContext<FamilyBookingDbContext>(options => options.UseNpgsql(connectionString, postgres => postgres.MigrationsAssembly(typeof(FamilyBookingDbContext).Assembly.FullName)));
...
if (builder.Configuration.GetValue<bool>("Database:MigrateOnStartup"))
{
    ...
    await scope.ServiceProvider.GetRequiredService<FamilyBookingDbContext>().Database.MigrateAsync();
