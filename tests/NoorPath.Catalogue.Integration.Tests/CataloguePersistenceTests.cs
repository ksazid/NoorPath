...
public sealed class CataloguePersistenceTests
{
    ...
    public async Task Migration_applies_and_publication_is_atomic_and_durable()
    {
        ...
        Assert.Equal(BatchStatus.Published, (await db.Batches.SingleAsync(cancellationToken: TestContext.Current.CancellationToken)).Status);
        Assert.Single(await db.PublicationAudits.ToListAsync(cancellationToken: TestContext.Current.CancellationToken));
    }
}
